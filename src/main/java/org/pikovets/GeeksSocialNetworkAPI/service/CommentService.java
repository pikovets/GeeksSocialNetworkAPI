...
public class CommentService {
    ...
    public Mono<Void> deleteComment(UUID commentId, UUID authUserId) {
        return commentRepository.findById(commentId).switchIfEmpty(Mono.error(new NotFoundException(COMMENT_NOT_FOUND))).filter(comment -> comment.getUserId().equals(authUserId)).switchIfEmpty(Mono.error(new NotAllowedException(USER_NOT_ALLOWED))).flatMap(comment -> commentRepository.deleteById(commentId));
    }
}
